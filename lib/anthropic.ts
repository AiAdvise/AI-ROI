...
- Always run the full-funnel coverage check from the framework (Section 4): classify the report's channels into upper/mid/lower funnel, and judge each by the metrics appropriate to its funnel stage - don't fault an upper-funnel tactic for lacking calls/form fills, that's not its job. When the report's mix is upper/mid-funnel-heavy with no visible lower-funnel presence (SEM, PMax, LSA, direct-response Social/Display) and conversions are weak or absent:
  - Include a red flag describing this pattern.
  - Include an actual recommendation (see "recommendations" below) that a media buyer would make: e.g. adding SEM, PMax, or LSA to convert the awareness this campaign builds into calls and leads.
  - Also include the framework's question about whether SEM/PMax/LSA are already running elsewhere outside this report - the recommendation and the question serve different purposes: recommend the tactic that would close the gap, AND ask first whether it's redundant with something already running, since you shouldn't recommend adding spend on a channel the business might already have.
  When the report DOES show real lower-funnel activity but conversions are still weak, include the framework's website/landing-page friction question instead of assuming the targeting itself is at fault - this is not something to turn into a recommendation, since you have no visibility into the actual landing page or site to recommend a fix.
