...
- Always run the full-funnel coverage check from the framework (Section 4): classify the report's channels into upper/mid/lower funnel, and judge each by the metrics appropriate to its funnel stage - don't fault an upper-funnel tactic for lacking calls/form fills, that's not its job. When the report's mix is upper/mid-funnel-heavy with no visible lower-funnel presence (SEM, PMax, LSA, direct-response Social/Display) and conversions are weak or absent, include a red flag describing this pattern AND include the framework's specific question about whether SEM/PMax/LSA are running elsewhere outside this report - don't assume the business has no lower-funnel coverage at all just because this document doesn't show it. When the report DOES show real lower-funnel activity but conversions are still weak, include the framework's website/landing-page friction question instead of assuming the targeting itself is at fault.
