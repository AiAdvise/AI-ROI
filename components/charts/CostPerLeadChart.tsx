import type { TrendPoint } from "@/lib/kpiTrends";
...
export default function CostPerLeadChart({ points }: { points: TrendPoint[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full h-auto" role="img" aria-label="Cost per lead over time">
      {gridLines.map((g) => {
      ...
      <path d={linePath} fill="none" stroke="#a0266b" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />

      {points.map((p, i) => {
        const { x, y } = xy(i, p.value);
        return (
          <circle key={i} cx={x} cy={y} r={4} fill="#a0266b" stroke="#ffffff" strokeWidth={2}>
            <title>{`${p.label}: $${p.value.toFixed(2)} per lead`}</title>
          </circle>
        );
      })}
